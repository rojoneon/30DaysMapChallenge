...
vars <- datos_complemen %>% 
  dplyr:: filter(mun=="039" | mun=="070" | mun=="097" | mun=="101" | mun=="098" | mun=="120")%>%
  dplyr::select(CVEGEO,viv5_r, edu49_r, salud5_r, viv36_r)

ageb_jal<-merge(x=ageb_jal,y=vars,by="CVEGEO")
...
quantiles_hacin <- ageb_jal %>%
  pull(viv5_r) %>%
  quantile(probs = seq(0, 1, length.out = 4))

# create 3 buckets for edu
quantiles_edu <- ageb_jal %>%
  pull(edu49_r) %>%
  quantile(probs = seq(0, 1, length.out = 4))
...
map <- 
  ...
  labs(x = NULL,
       y = NULL,
       title = "Segregación espacial bivariada, ZMG",
  ...
  coord_sf(xlim = c(-103.52, -103.2), ylim = c(20.5,20.82), expand = FALSE) +
  # add the theme
  theme_map()
